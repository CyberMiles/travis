...
mkdir -p centos && cd centos \
  && wget https://github.com/CyberMiles/libeni/releases/download/v1.2.0/libeni-1.2.0_centos-7.4.tgz -O libeni.tgz \
  && tar zxvf *.tgz --strip-components 1 && cp /app/travis .

zip "/scripts/travis_${BUILD_TAG}_centos-7.4.zip" travis lib/*
